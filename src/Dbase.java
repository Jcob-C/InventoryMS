...
public class Dbase {
    ...
    static String[][] inserted_into(String[][] table, String[] row) {
        ...
        String[][] 
        new_table = new String[table.length+1][table[0].length];
    ...
    static void add_product(String[] row) {
        ...
        sales = inserted_into(products, sales_row);
    ...
    static void log_activity(String username, String activity) {
        String[] 
        log_row = new String[] {saved_date+"|"+get_time(),username,activity};
        
        if (activity_log.length > ACTIVITY_LOG_LIMIT) {
            activity_log = removed_from(activity_log, 1);
        }

        activity_log = inserted_into(activity_log, log_row);
    }
    ...
    static void daily_update() {
        ...
        if (saved_date == null) { // if first program start up ever
            saved_date = current_date; 
        } 
        else if (!current_date.equals(saved_date)) { // if date is not current

            saved_date = current_date;

            for (r = 1; r < sales_rows; r++) {
                update_history(r, sales[r][2]);
                sales[r][2] = "0";
                sales[r][3] = ""+ Utils.total_sales(sales_history[r], 7);
                sales[r][4] = ""+ Utils.total_sales(sales_history[r], 30);
            }
        }
    }
    ...
    static void save_all() {
        ...
        try {
            FileWriter writer = new FileWriter("others.txt");
    ...
    static void load_all() {
        ...
        try {
            BufferedReader reader = new BufferedReader(new FileReader("others.txt"));
    ...
    // doesnt save first row (column names row)
